...
void ProcessSpork(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "spork") {
        ...
        if (spork.nTimeSigned >= Params().NewSporkStart() && Params().SporkKeyOld().size()>0) {
...
bool CSporkManager::CheckSignature(CSporkMessage& spork, bool fCheckSigner)
{
    ...
    if (!fValidWithNewKey && GetAdjustedTime() < Params().RejectOldSporkKey() && Params().SporkKeyOld().size()>0) {
